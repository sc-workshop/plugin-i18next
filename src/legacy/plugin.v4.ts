...
export const pluginV4: InlangPlugin<{
	...
	loadMessages: async ({ settings, nodeishFs }) => {
		settings["plugin.neko.i18next"].variableReferencePattern = settings[
			"plugin.neko.i18next"
		].variableReferencePattern || ["{{", "}}"];
		return loadMessages({
			settings,
			pluginSettings: settings["plugin.neko.i18next"],
			nodeishFs,
		});
	},
	...
	addCustomApi: ({ settings }) =>
		ideExtensionConfig(settings["plugin.neko.i18next"]),
};
...
async function getFileToParse(
	...
	try {
		...
		if (Object.values(obj).some((value) => typeof value === "object")) {
			hasNestedKeys = true;
		}
